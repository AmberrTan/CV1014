# Fetch Gyms Unified Script Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the OSM import/enrichment scripts with a single `scripts/fetch_gyms.py` CLI that uses `requests`, runs import + enrichment sequentially, and overwrites the database by default while updating docs/dependencies.

**Architecture:** A single CLI script contains a small `requests` helper (session + JSON request with retries) and merges the existing Overpass import and Nominatim enrichment logic. The script writes the final enriched dataset to the target JSON path (defaulting to `data/gyms.json`).
...
- Create: `scripts/fetch_gyms.py`
- Create: `tests/test_fetch_gyms.py`
- Modify: `README.md`
- Modify: `requirements.txt`
- Modify: `pyproject.toml`
- Delete: `scripts/import_osm_gyms.py`
- Delete: `scripts/osm_import.py`
- Delete: `scripts/openstreetmap_enrichment.py`

---

### Task 1: Add Failing Unit Tests For The New Helper Functions
...
Run: `uv run pytest tests/test_fetch_gyms.py -q`

Expected: FAIL with `ModuleNotFoundError: No module named 'scripts.fetch_gyms'`.

- [ ] **Step 3: Commit the failing tests**

```bash
git add tests/test_fetch_gyms.py
git commit -m "test: add failing tests for fetch gyms helper"
```

---

### Task 2: Implement `scripts/fetch_gyms.py` (Unified Script + Requests Helper)

**Files:**
- Create: `scripts/fetch_gyms.py`

- [ ] **Step 1: Implement the unified script**
...
from gym_recommender.data import DEFAULT_DATABASE_PATH, load_database, save_database
...
- [ ] **Step 3: Commit the unified script**

```bash
git add scripts/fetch_gyms.py
git commit -m "feat: add unified fetch gyms script"
```

---

### Task 3: Add the Requests Dependency
...
- [ ] **Step 2: Add `requests` to `pyproject.toml`**
...
### Task 4: Update Docs + Remove Old Scripts
...
- [ ] **Step 1: Update the README data import section**
...
```bash
rm scripts/import_osm_gyms.py scripts/osm_import.py scripts/openstreetmap_enrichment.py
```
...
- [ ] **Step 4: Commit doc + cleanup changes**

```bash
git add README.md scripts
git commit -m "docs: switch OSM import to fetch_gyms"
```
...
1. **Spec coverage:**
   - Unified script with requests helper: Task 2
   - CLI + overwrite default: Task 2 + Task 4
   - Docs updated: Task 4
   - Dependencies updated: Task 3
   - Old scripts removed: Task 4

2. **Placeholder scan:**
   - No TODO/TBD placeholders included.

3. **Type consistency:**
   - `GymRecord` and `OpenStreetMapData` types align with `gym_recommender.models`.
   - CLI flag names match internal parameter names.

---

## Execution Handoff

Plan complete and saved to `docs/superpowers/plans/2026-04-11-fetch-gyms-plan.md`. Two execution options:
